...
const DOC_HINTS: Partial<Record<DocumentType, string>> = {
  ...
  lease_agreement:
    "The lease agreement shows monthly rent (sale_price for leases), lease_start_date/lease_end_date, tenant and landlord names and contact info, and deposit details. On OREA Form 400, lease_start_date comes from section 2, TERM OF LEASE, immediately after the word 'commencing' (not the Agreement dated line at the top). On OREA Form 400 page 1, mls_number is the standalone alphanumeric listing code near the far right of section 1, PREMISES/property address line; do not confuse it with the postal code beside the city/province. For lease packages, closing_date must use the lease_start_date from that Term of Lease commencing line.",
  form_320_confirmation_cooperation:
